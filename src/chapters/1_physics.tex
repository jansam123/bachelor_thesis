...
To illustrate this, we take a look at the fraction of cross section of $e^+ e^- \rightarrow$ hadrons and $e^+ e^- \rightarrow \mu^+ \mu^-$
\begin{equation}
...
These interactions are non-trivial, a theory called \emph{gluodynamics} is developed to describe just gluon-gluon interactions.
...
We can see this directly from the scattering amplitude (\ref{eq:IR_sing}), where in the dominator we have
\begin{equation}
    \label{eq:IR_sing}
    (p_1+k)^2 = 2E\omega(1- \cos{\theta}),
\end{equation}
where we consider $p_1 = (E, 0,0,E)$, a hard quark (energtic quark, whose mass can be neglected), and $k = \omega (1, 0, \sin{\theta}, \cos{\theta})$, $\theta$ being the angle between the quark and the gluon.
...
The KLN theorem \cite{IR_sing_K,IR_sing_LN} states: \emph{When all the possible interactions are considered, the scattering amplitude is finite}, ie. is \textbf{infrared (IR) and collinear safe}. 

The same process happens for gluons, they can emit another gluon and the scattering amplitude goes to infinity.
...
This is called \emph{gluon radiation} and is the reason why the coupling constant grows with the distance between the quarks.
...
In this context, it is good to introduce the word \emph{parton}, which is a generic term for a quark or a gluon.
As we have seen in \cref{sec:IR_div}, the less energetic the parton is, the more likely it will emit gluons.
Meaning that when partons reach some energy scale, they become bound, effectively making the partons almost immediately bound after hard scattering processes.
...
\end{itemize}
\emph{Mesons} are hadrons made up of one quark and one antiquark, whereas \emph{baryons} are made up of three quarks.
...
Here the gluons are split into quarks and antiquarks, which are then rearranged to \emph{clusters} with zero net color charge.
Both approaches are illustrated in \cref{fig:hadronisation}.
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.5\linewidth]{src/img/hadronization.png}
    \caption[Illustration of the hadronization models. The left one is the Lund model, and the right one is the cluster fragmentation.]{Illustration of the hadronization models. The left one is the Lund model, and the right one is the cluster fragmentation. \footnotemark}
...
\section{Jets}
\label{sec:jet}
As we have seen in \cref{sec:IR_div} partons emit soft and collinear gluons.
...
Take as an example a quark that initially radiates one collinear gluon, having approximately the same momentum as the quark.
...
The reason why is this important is that if the jets are not defined correctly, the infinities in the scattering amplitude will not be canceled out, as is stated in the KLN theorem \cite{IR_sing_K,IR_sing_LN}.
This makes analyzing the data very difficult. 

There are complex algorithms that use special physical variables to enable the reconstruction of the jets \cite{antikt}.
...
\begin{figure}[htb]
    ...
    \caption{Collinear unsafe jet algorithm. Initially, there are two jets, but if one jet radiates a soft gluon, they are put together.}
