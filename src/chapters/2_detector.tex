...
It covers way more volume than the pixel detector, \green{delete:[providing the most precise tracking system]} with a precision of up to 25 $\mu$m.
...
\section{Trigger}
\label{sec:trigger}
The high luminosity that \LHC provides results in an interaction rate of \green{40 MHz}, equivalent to 60 TB/s of data throughput \cite{trig}.
...
\green{The trigger system consists of two levels, \emph{L1} and \HLT \cite{trig}.
L1 uses data from calorimeters and muon spectrometers defining a \RoI, where it performs a decision on whether to keep the event or not, reducing the interaction rate to 100 kHz.  
Fast custom electronic hardware with a latency of 2.5$\mu$s is used.}

\green{The \HLT trigger uses the \RoI defined by L1 but with more precise data from the detector in that region.
Using a CPU farm of 40k CPU cores, \HLT reduces the rate up to 6.9 kHz, with a decision time of 300$\mu$s.}
