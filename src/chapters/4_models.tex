...
They can not only learn the differences between signal and background but also \emph{general physical concepts} \cite{qcd_as_nlp}. 

\dl builds from the \mlp \cite{mlp}, which is a multi-layered neural network able to learn a \emph{any} non-linear function from input data.
...
The \emph{backward pass} or \emph{backpropagation}\footnote{Term backpropagation comes from the application of the chain rule. Imagine the model as a composite of many functions. As you compute the derivative of the loss wrt. some parameter, you have to use the chain rule to get \emph{back} to that parameter.} is the process of adjusting the parameters $\pmb{w}$ to fit the desired output $\pmb{y}$.
...
Before we start discussing the individual models, we will briefly touch on the training process as a whole.

The dataset is split into a \emph{training set}, a \emph{validation set}, and in our case also a \emph{test set}.
The training set is used to train the model, the validation set is used to monitor the training process, and the test set is used to evaluate the performance of the model.
The training process is split into several \emph{epochs}, where each epoch consists of several \emph{iterations}.
An epoch is a full pass through the training whole data, while an iteration is a single batch of data.
The total number of \emph{steps} is the product of the number of epochs and the number of iterations. 

After each epoch, we evaluate the model on the training set and the validation set.
Validation is done to have a statistically independent measure of the performance of the model during the training process.
...
The output of the model is a vector of numbers $\pmb{v}$, where the length of the vector is the number of classes.
It is a \emph{one-hot} encoded vector, meaning that the index of the non-zero element is the desired class.
To perform a classification task, we need to convert the vector $\pmb{v}$ into a probability distribution over the classes.
...
\end{equation}
Ideally, we would like the vector $\pmb{v}$ to have a single non-zero element, which would correspond to the correct class. 
...
The output of the model is a vector of numbers $\pmb{v}$, where the length of the vector is the number of variables to be predicted.
There is no output layer function for regression.


\subsection{Loss Functions}
\label{sec:loss_functions}
The loss function is a measure of how far the model is from the target.
...
To give a brief introduction to cross-entropy loss, we will introduce some basic concepts from information theory \cite{shannon}.
...
\begin{description}
    ...
    \end{equation}
    The self-information is a measure of \emph{surprise} of obtaining random variable $x$ when sampled from $p$.
    ...
    \end{equation}
    The entropy is a measure of the \emph{average surprise} of obtaining a random variable from the distribution $p$.
    The entropy is zero if the random variables are deterministic, and it is maximal if the random variables are uniformly distributed.
    ...
    \end{equation}
    The cross-entropy is a measure of the \emph{average surprise of distribution $q$} when the actual distribution is $p$.
    ...
    \item[Kullback-Leibler (KL) divergence] is defined as a difference between the cross-entropy of the distribution $p$ and $q$ and the entropy of the distribution $p$.
    \begin{equation}
        D_{\text{KL}}(p|q) =  H(p,q) - H(p) = - \sum_{x \in X} p(x) \log \frac{p(x)}{q(x)}.
    \end{equation}
    The KL divergence is a measure of the \emph{excess of the average surprise of distribution $p$} when the actual distribution is $p$.
\end{description} 
...
\end{equation}
It is important to note that cross-entropy is defined for probability distributions, so the cross-entropy loss can be used only for classification tasks.

This is a good place to discuss the similarities to physical entropy.
...
It is the \emph{measure of ignorance}, in a sense of missing information.
In this context, we can say that the cross-entropy is the \emph{measure of ignorance} of the model.
In quantum mechanics, entropy was developed by John von Neumann \cite{VonNeumann} where the definition is a little bit different, but in terms of probability distributions it is the same.
The KL divergence is precisely the \emph{measure of the distinguishability of two quantum states}.


\subsection{Optimizers}
\label{sec:optimizers}
\emph{Optimizers} are algorithms that adjust the weights of the model to minimize the loss function.
...
\begin{description}
    ...
    Second, Adam uses the \emph{adaptive learning rate} to avoid the problem of the learning rate being too small or too large, which is represented by the division of $\sqrt{\pmb{r}}$ in line 7 of \cref{alg:adam}.
    To calculate the adaptive learning rate, Adam uses the second moment $\pmb{g}^2$, line 4 of \cref{alg:adam}.
    ...
    This normalization is done layer-wise and is to ensure that the weights will not diverge.
...
Until now learning rate was a constant, but it can be changed during the training process.
This is called \emph{learning rate scheduling}.
...
Huge models tend to diverge, which is called \emph{exploding gradient}.
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Sigmoid} - after the last layer to convert a number to probability 
    \begin{equation}
    ...
    \end{equation}
\end{itemize}
Sigmoid and Softmax were already discussed in \cref{sec:output_layer}.
...
These terms are generally used across all \ml algorithms, but they are especially important in \dl, where the number of parameters is very large, much more than the number of training examples (for example \cite{deit3,gpt3}).
...
However, to be able to not overfit the model, we need to use \textbf{regularization}. 
...
\begin{description}
    ...
    This regularization is usually implemented into the optimizer because the gradient can be explicitly calculated, see section \ref{sec:optimizers}. 
    ...
    Dropout forces the network to learn different representations of the data and to not rely on individual neurons.
    ...
    For example, if we have an image as an input, we can randomly rotate, crop, change the brightness, etc.
...
\subsection{Metrics}
\label{sec:metrics}
A \emph{metric} is a measure of how well the model performs, it \emph{evaluates} the model.
We will only discuss the metrics for binary classification since that is what we will be using.
The output of a binary classification model is a single number between zero and one $o \in [0,1]$, where 0 corresponds to the first class and 1 corresponds to the second class.
...
We try to simplify the notation by using the terms \emph{true} $T$ and \emph{false} $F$ to denote the correct and incorrect classification, respectively, and the numbers $0$ and $1$ to denote the output class.
...
\begin{description}
    ...
    \end{equation}
    There are more terms for this metric if $i = 1$: \emph{true positive rate} (TPR) sensitivity, recall, hit rate, probability of detection...
    \item[Rejection] $\epsilon^{-1}_i$ of class $i$ is the inverse of fraction of correctly identifying the class $i$
    \begin{equation}
        \label{eq:rejection}
        \epsilon^{-1}_i = \frac{1}{\epsilon_i} = \frac{T_i + F_i}{T_i}.
    \end{equation}
    \item[False Rate] $1 - \epsilon_i$ of class $i$ is the fraction of incorrectly identifying the class $i$
    \begin{equation}
        \label{eq:false_rate}
        1 - \epsilon_i = \frac{F_i}{T_i + F_i}
    \end{equation}
    There are more terms for this metric if $i = 1$: \emph{false positive rate} (FPR), fall-out, probability of false alarm...
\end{description}

Following metrics are defined with a \emph{variable threshold}:
\begin{description}
    \item [ROC] (Receiver Operating Characteristic) curve is a plot of the efficiency of the second class $\epsilon_1$ against the false rate $1 - \epsilon_0$ of the first class for different thresholds. 
    It is good to note that there is no point in switching the classes since the ROC curve would just reflect wrt. the diagonal. 
    \item[AUC] is the Area Under the ROC Curve, which is a simple integration of the ROC curve.
    \item[Rejection at Efficiency] is the rejection of the one class at a given efficiency of the second class with an adjusted threshold.
\end{description}
...
\emph{Fully Connected Network} (FC) \cite{mlp} is the simplest neural network, where all the neurons in one layer are connected to all the neurons in the next layer and previous layers.
...
The gate is a \emph{learnable} function that decides how much of the input is passed through the network and how much is bypassed.
...
The trick is in estimating the probability $p$ of using the newly calculated output rather than the input.
...
They are developed specifically for \HEP purposes, using the theory of \emph{Deep Sets} \cite{deep_set}.
...
In the case of \EFN, the summation over the \PFOs is done as a matrix multiplication over the \PFO index, the output is a matrix (because both $\pmb{z_i}$ and $\pmb{\varphi_i}$ are matrices) which is then flattened to a vector.\footnote{This is done using the \texttt{tf.einsum} function, hence the name in \cref{fig:pfn_efn}, followed by a \texttt{tf.reshape}.}
This is not the case in the original paper, but the performance is much better with this modification, still satisfying the IRC conditions and the form of (\ref{eq:efn}).
...
It is based on the \emph{Attention} mechanism which is a core component of the \emph{Transformer} network allowing the elements of the set to \textbf{communicate} with each other.
The overall structure of the original \trans consists of \emph{Encoder} and \emph{Decoder} blocks, but in our use case, we only use the \emph{Encoder} part, which is shown in \cref{fig:trans}.
The Encoder part is supposed to form a \emph{representation} of the input data based on the task.
The Doceder part on the other hand reconstructs some output data from the internal representation of the Encoder part.
To give a more concrete example imagine a translation between languages, say for example Czech and Slovak.
The encoder forms a representation of what the input Czech sentence is trying to say, i.e. the abstract information, and the decoder reconstructs the Slovak sentence from the internal representation of the encoder.
...
The \trans architecture provides a way for \textbf{PFOs to communicate} and exchange valuable information, creating a more complete representation of the jet (input data).
Despite its complexity, the \trans provides a bigger advantage over the \PFN and \EFN, which is the ability to \textbf{learn long-range dependencies} in the input data.
...
In the case of general Attention for $Q$, $K$, and $V$ can be the input data different (some other input is asking the question, and some other input has the keys, and some other the values).
Self-Attention is a special case of Attention where $Q$, $K$, and $V$ are created from the same input, i.e. the input data is asking itself the questions, and the input data is the answer to the questions.
Going further in this section, we will only focus on \SA.

For visual aid, the \MHSA layer is shown in \cref{fig:trans_layers} (a). 
...
To extract the answer to the question, the \texttt{Attention} is multiplied by the $V$ matrix, resulting in a matrix of shape $B \times N \times C$, being the same shape as the input data.

Instead of doing the attention mechanism across the whole embedding dimension, the \trans uses multiple \emph{heads}.
Each head is a separate attention mechanism working with the head dimension $d = C / H$, where $H$ is the number of heads. 
The input and output shape of each head is $B \times N \times C/H$, constructed by reshaping the input data to $B \times H \times N \times C/H$ (the calculations across the heads are done in parallel, hence the extra dimension).
To get the final output, the heads are concatenated together, resulting in a matrix of shape $B \times N \times C$.
As a result, the argument of attention must be scaled by $\sqrt{d}$, as stated in \cref{eq:attn}.

\subsection{Feedforward Network}
\label{sec:ffn}
The \emph{Feed Forward Network} (FFN) is a simple network with two \texttt{Linear} layers with \texttt{Activation} in between.
Its main purpose is to allow the individual vectors to learn from the data they obtained from other vectors in the \MHSA layer. 
...
After the \MHSA is the dropout layer preventing overfitting.
...
The \SA Block are stacked on top of each other, constructing $L$ layers, which are the heart of the \trans.

\subsection{Class Extraction}
\label{sec:class_extract}
The \trans is a general-purpose model, capable of learning any type of representation.
...
It passes through the \trans layers together with the input vectors extracting and storing the class information.
...
\begin{figure}[htb]
    ...
    \caption{Embedding layers. (a) is a per input vector embedding, (b) is the embedding of interaction variables.}
    \label{fig:embed}
\end{figure}
...
These variables are manually engineered\footnote{The explicit form depends on the type of input. In the case of PFOs, the variables are shown in \red{[????]}} and embedded using the \emph{CNN embedding}, consisting of multiple blocks of \pointCNN, batch normalization and activation, as shown in \cref{fig:embed} (b).
...
To make the addition possible, the \pointCNN embedding of interaction variables is done into $H$ (number of heads) dimensions, resulting in a matrix of shape $B \times N \times N \times H$ providing each head with its own interaction variables matrix $U$ of shape $B \times N \times N$.

The only difference between \SA Blocks in \trans and \ParT is the addition of two \LN, one is right after the \MHSA and the other is right before the second \texttt{Linear} layer of the \FFN.
...
The output class is calculated from the Class Token the same way as in \trans.

Utilizing the interaction variables helps the model to focus on the important parts of the input data.
Since the interaction variables are embedded into a small dimension compared to the input vectors, which are distributed to individual heads of the \MHSA allowing each of them to focus on a different part of the input data.
...
The class token is concatenated to this set of vectors, resulting in a tensor of shape $B \times (N+1) \times C$.
On top of that the Class Token is used as a query for the \MHA, as seen in \cref{fig:part_CA}.
This allows the model to focus on the class information, the Class Token asks for it and stores it.
...
The output of the second \CA Block is a vector of shape $B \times C$, with the extracted information about the class of the entire input, which is then passed through a \texttt{Linear} layer to get the final output followed by sigmoid or softmax activation.
...
We do this twice in the \MHA, once right after the matrix multiplication of the query and key, and once right before the matrix multiplication of the value and the attention weights, as shown in \cref{fig:depart_SA}.
...
The deeper the layer, the bigger the probability of dropping it, linearly dependent in the form
\begin{equation}
    \label{eq:stoch_depth}
    p_l = \frac{p_L}{2} \cdot \left(1 + \frac{l}{L}\right),
\end{equation}
where $p_L$ is the probability of dropping the last layer, $L$ is the total number of layers, and $l$ is the current layer number.
...
\emph{Layer Scale} \cite{cait} is a type of normalization, which helps the convergence of Transformers with many layers. 
