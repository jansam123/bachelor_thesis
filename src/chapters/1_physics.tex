...
\end{enumerate}
\end{multicols}
All quarks can interact \green{not only} weakly and electromagnetically \green{, but also} via the \emph{Strong Force}, mediated by \emph{gluons}.
...
\end{itemize}
\green{Higgs boson is a scalar boson, giving invariant mass to other particles.} 
...
In \QED,\green{[delete: we have the \emph{asymptotic freedom}, which means ]}the coupling constant $g$ goes to zero as the distance between the particles goes to infinity.
