...
Previous \green{ATLAS} work primarily used only one variable to tag the jet \cite{ntrk_tag}.
...
\green{ATLAS \MC samples do not come with truth information about pileup to reduce the amount of used disk space,} so we filter them out as much as possible using the abovementioned cuts.
...
Combining JZ slices with equal probability ensures the model works on the full spectrum of jets \green{and \emph{does not use} $\pT$ to make the decision.}
...
\begin{figure}[!htb]
    ...
    \end{subfigure}
    \caption{Normalized $\pT$ distributions of jets \green{before} the JZ cuts are applied.}
...
\begin{itemize}
    ...
    \item \textbf{\PFO variables} - variables that are \green{construsted from} the 4-momenta of the \PFOs, they are calculated for \emph{each PFO} in a given jet.
...
Most of the \PFOs \green{are neutral particles or cannot be associated with massive particles, so we assume they are relativistic and neglect their masses.}
...
Again we take the \emph{logarithm of all these variables} \green{(except $z$ because it is bounded, see the distribution in \cref{sec:app_pfo_int_variables})} to squish the distributions. 
